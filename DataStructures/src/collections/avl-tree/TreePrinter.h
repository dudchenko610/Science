...
namespace treeprinter 
{
    ...
    public:
        ...
        static display_rows get_row_display(avltree::Node<T>* root) {
            // start off by traversing the tree to
            // build a vector of vectors of Node pointers


            vector<avltree::Node<T>*> traversal_stack;
            vector< std::vector<avltree::Node<T>*> > rows;
            if (!root) return display_rows();

            avltree::Node<T>* p = root;
        ...
        static void Print(avltree::Node<T>* root, int maxDepth) {
            const int d = maxDepth;

            // If this tree is empty, tell someone
            if (d + 1 == 0 || root == nullptr) {
                cout << " <empty tree>\n";
                return;
            }
